...
- `server/src/content.rs` validates versioned original world content loaded from `server/data/world.json` (or explicit `CONTENT_PATH`) and produces an admin-visible content manifest. Promoted standalone worlds pair that path with an explicit `TERRAIN_DETAIL_AUTHORITY_PATH`; the local world runner also gives each world a distinct journal and settlement outbox.
...
- Terrain snapshots expose independent `detailAuthorityEnabled` and
  `visualDetailEnabled` switches. The former controls server-backed blockers,
  resources, decay consumers, and procedural interiors; the latter controls
  approved scenic composition. A client rendering scenery while authority is
  disabled must strip gameplay metadata instead of creating local authority.
...
- `server/src/sim/actor_intents.rs` is the shared authoritative speech gateway for players and NPCs. WebSocket player speech, schema-checked Animus output, and deterministic canned fallback all become the same typed actor intent; the simulation owns sanitation, identity checks, audience proximity, and mutation. Cognition remains outside the tick and has no privileged gameplay write path.
...
`INTEREST_RADIUS` controls the WebSocket snapshot radius around each player. The default is `1800` world units, which covers the plan-oblique camera through a 1080p viewport plus an offscreen arrival margin so authoritative actors and props do not enter snapshots after they are already visible. Lower values reduce payload size and object/player fanout but must be coordinated with the supported camera/viewport envelope or visible pop-in returns; `/api/snapshot` remains full-world and admin/debug only. `/metrics` reports total outbound WebSocket bytes plus last/max serialized message and snapshot byte sizes so shard operators can see payload growth before it becomes a capacity issue.
...
- Approved chunked shards load `TERRAIN_CHUNK_INDEX_PATH`, verify its server-content SHA-256 pin and bounded chunk metadata, verify every chunk byte count/hash, and assemble only one bounded regional fixed-point terrain authority. The hard 262,144-tile region ceiling prevents accidental continent-scale ECS processes. Cross-region player routing remains outside the current single-shard process.
- World-package manifest v4 pairs authority chunks with gameplay-resolution visual controls carrying the same sample aprons. Validation decodes every control, recomputes cardinal overlap hashes, and rejects pixel drift even when an altered image and containing index were rehashed. Accepted illustrated output receives the same validation and promotion treatment. Approved clients skip the gameplay monolith, hash-verify separate authority and illustrated indexes, prefetch matching 3 by 3 windows, compose shared samples, and evict both caches under fixed LRU bounds. Controls are generation inputs and are never rendered as approved art.
- `worldgen:regions` is the single-host durable generation scheduler. It pins atlas and template identity in an atomic batch journal, caps fanout, retries bounded failed work through region quarantine, and revalidates the exact manifest hash of every completed region before skipping it on resume. It is intentionally generation-plane tooling, not gameplay authority or a substitute for the live shard router.
- `worldgen:preview` stages a validated package in a hash-addressed local asset root with a `review` registry/manifest and isolated content, authority, journal, and outbox. The browser accepts review state only with `preview=1`. This exercises the real chunked client/server path without weakening or writing through the human-approved promotion boundary.
- Atlas-region shards carry the parent atlas hash, region coordinate, global tile origin, and exact cardinal neighbor IDs through server content and snapshots. Outward edge pressure emits one latched handoff intent with global crossing and neighbor-local destination coordinates. `sim/transfer.rs` now exports and validates bounded identity, deed, inventory, lifecycle-age, and destination state between reciprocal neighbors. `region_transfer_tokens.rs` signs that state with a short-lived HS256 ticket bound to source, destination, player, atlas identity/hash, and a one-use nonce; tests reject replay, expiry, signature drift, wrong atlas, wrong destination, unknown items, oversized stacks, and duplicate destination admission. The live tick loop still only audits intents. Runtime wiring must arrive with a trusted region endpoint registry, source freeze/ack state machine, and shared durable replay ledger so a process crash cannot lose or duplicate a player between nonce consumption and ECS admission.
