...
- `PUBLIC_DEPLOYMENT=true` refuses startup when session tickets, account auth, admin auth, metrics auth, synced durable writes, token quality, token placeholders, or Origin allowlisting are left in local-dev mode.
