...
Set `PUBLIC_DEPLOYMENT=true` for any shared environment. In that mode the server fails startup unless `DEPLOYMENT_PROFILE=shared-poc` or `production`, `REQUIRE_SESSION=true`, `REQUIRE_ACCOUNT=true`, `DURABLE_SYNC_WRITES=true`, a strong bounded non-placeholder account credential for the selected `ACCOUNT_AUTH_MODE`, strong distinct bounded non-placeholder `ADMIN_TOKEN` and `METRICS_TOKEN` values, and bounded exact `ALLOWED_ORIGINS` are all configured. Public JWT mode also requires bounded identity config: `ACCOUNT_JWT_ISSUER` must be a non-local HTTPS issuer URL without query, fragment, userinfo, whitespace, or control characters, and `ACCOUNT_JWT_AUDIENCE` must be bounded, printable, and non-placeholder. `scripts/public-deployment-smoke.js` verifies the fail-closed startup path and the hardened live behavior.

Set `DEPLOYMENT_PROFILE=local`, `shared-poc`, or `production` to make runtime intent auditable. The default is `local`, and public deployment rejects that default. `shared-poc` requires `PUBLIC_DEPLOYMENT=true`; `production` intentionally refuses startup until durable database/event-store, signer/indexer isolation, and cross-process admission/rate-limit state are implemented. `scripts/production-profile-guard-smoke.js` verifies that fail-closed runtime path.

Run `npm run preflight:deployment` before booting shared PoC deployments. The preflight checks the intended public-mode environment without starting the server and fails on a missing or mismatched `DEPLOYMENT_PROFILE`, missing strict sessions, account auth, synced durable writes, shared journal/outbox paths, weak, placeholder, or reused credentials, invalid JWT issuer/audience config, invalid Origins, malformed `BIND_ADDR` socket addresses, missing or malformed `GIT_SHA` build provenance, chain-stub exposure, non-integer or oversized capacity/payload budgets, inconsistent admission/rate-limit budgets, and accidental drain mode. The server repeats the runtime budget envelope at startup, so bypassing preflight still rejects runaway capacity/payload knobs, peer caps above shard caps, rate-limit bursts above per-minute refill budgets, and invalid WebSocket timing. Its `production` profile intentionally fails until durable persistence, signer/indexer isolation, and cross-process admission/rate-limit state are implemented; the identity blocker clears only for configured JWT mode with valid issuer/audience config, not the temporary dev-token mode. Intentional rollback or shard-removal boots with `DRAINING=true` must pass `--allowDraining` so the maintenance posture is explicit in deploy automation.
...
- `PUBLIC_DEPLOYMENT=true` refuses startup when `DEPLOYMENT_PROFILE` is still local, or when session tickets, account auth, admin auth, metrics auth, synced durable writes, token quality, token placeholders, or Origin allowlisting are left in local-dev mode.
