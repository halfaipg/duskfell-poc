...
- Gameplay-affecting resource gather, organic mycelium feeding, and starter craft journal events are covered by a restart/replay smoke so inventory mutations cannot silently become live-only evidence. Gatherable grove, ore, mycelium shrine, and ancient ruin nodes now expose server-owned remaining/max amounts plus lifecycle state in snapshots, and resource-node changes are journaled/replayed so depletion survives restart. The next hardening step is durable per-tile ecology once terrain details become authoritative gameplay entities.
