...
const port = Number(args.port ?? 4140);
const runtimeDir = path.resolve("var", "session-expiry-smoke");
...
try {
  ...
  const afterExpirySummary = await fetchJson("/admin/summary");
  ...
  result = {
    ...
    beforeExpiry: {
      ...
      sessionTicketCapacity: beforeExpirySummary.sessionTicketCapacity,
    },
    afterExpiry: {
      sessionPendingTickets: afterExpirySummary.sessionPendingTickets,
      sessionTicketCapacity: afterExpirySummary.sessionTicketCapacity,
    },
    metrics,
    ...
    ok:
      ...
      beforeExpirySummary.sessionTicketCapacity === 1 &&
      afterExpirySummary.sessionPendingTickets === 1 &&
      afterExpirySummary.sessionTicketCapacity === 1 &&
      metrics.sundermere_session_pending_tickets === 1 &&
      metrics.sundermere_session_ticket_capacity === 1 &&
      metrics.sundermere_session_tickets_issued_total === 2 &&
      metrics.sundermere_session_ticket_capacity_rejected_total === 0,
  };
} finally {
  if (server) {
