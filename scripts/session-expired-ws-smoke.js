...
const runtimeDir = path.resolve("var", "session-expired-ws-smoke");
...
try {
  ...
  const beforeExpirySummary = await fetchJson("/admin/summary");
  await sleep(1300);
  const upgrade = await requestWebSocketUpgrade(session.body.sessionToken);
  await sleep(150);
  ...
  result = {
    ...
    upgrade: {
      statusCode: upgrade.statusCode,
      statusLine: upgrade.statusLine,
      body: upgrade.body.trim(),
    },
    beforeExpiry: {
      players: beforeExpirySummary.players,
      activeConnections: beforeExpirySummary.activeConnections,
      sessionPendingTickets: beforeExpirySummary.sessionPendingTickets,
    },
    afterReject: {
      players: afterRejectSummary.players,
      activeConnections: afterRejectSummary.activeConnections,
    ...
    ok:
      session.status === 200 &&
      beforeExpirySummary.players === 0 &&
      beforeExpirySummary.activeConnections === 0 &&
      beforeExpirySummary.sessionPendingTickets === 1 &&
      upgrade.statusCode === 401 &&
      upgrade.body.includes("expired session ticket") &&
      afterRejectSummary.players === 0 &&
      afterRejectSummary.activeConnections === 0 &&
      afterRejectSummary.sessionPendingTickets === 0 &&
      metrics.sundermere_players === 0 &&
      metrics.sundermere_active_connections === 0 &&
      metrics.sundermere_ws_connections_total === 0 &&
...
async function requestWebSocketUpgrade(sessionToken) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      settled = true;
      socket.destroy();
      reject(new Error("timed out waiting for expired-ticket upgrade rejection"));
    }, 5000);
    let response = "";
