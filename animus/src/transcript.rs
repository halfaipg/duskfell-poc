...
/// Milestone conversation memory: a bounded, per-NPC ring of recent turns.
...
impl TranscriptStore {
    pub fn record(&mut self, npc_id: &str, speaker: &str, text: &str) {
        self.record_at(npc_id, speaker, text, Instant::now());
    }

    fn record_at(&mut self, npc_id: &str, speaker: &str, text: &str, now: Instant) {
        let transcript = self
            .transcripts
    ...
    pub fn turns(&self, npc_id: &str) -> Vec<Turn> {
        self.turns_at(npc_id, Instant::now())
    }

    fn turns_at(&self, npc_id: &str, now: Instant) -> Vec<Turn> {
        match self.transcripts.get(npc_id) {
    ...
    pub fn turn_count(&self, npc_id: &str) -> usize {
        self.turns(npc_id).len()
    }
}
