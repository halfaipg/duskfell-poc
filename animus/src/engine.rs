        ...
        match event {
            GameEvent::ActorSpoke {
                ...
                let job = Job {
                    ...
                    fallback: FallbackTrigger::Speech { actor_id },
                    npc_id,
                };
            ...
            GameEvent::PartyInvite {
                ...
                let job = Job {
                    ...
                    fallback: FallbackTrigger::PartyInvite { invite_id },
                    npc_id,
                };
                enqueue_interactive(&shared, &interactive_tx, job).await;
            }
            GameEvent::ActorLingered {
                ...
                let job = Job {
                    ...
                    fallback: FallbackTrigger::Greeting { actor_id },
                    npc_id,
                };
            ...
            GameEvent::PartyChanged { npc_id, detail } => {
                shared
                    .transcripts
                    .lock()
                    .await
                    .record(&npc_id, "[event]", &detail);
            }
            GameEvent::IntentRejected {
                ...
                shared.transcripts.lock().await.record(
                    &npc_id,
                    "[event]",
                    &format!("intent {decision_id} rejected: {reason}"),
                );
            }
        }
    }
...
async fn process_job(shared: &Arc<Shared>, job: Job) {
    ...
    let (transcript_turns, turns) = {
        ...
        (
            transcripts.turn_count(&job.npc_id),
            transcripts.turns(&job.npc_id),
        )
    };
    ...
    match complete_validated(shared, provider, request).await {
        Ok(intent) => {
            ...
            if let Some(text) = intent.params.get("text").and_then(|text| text.as_str()) {
                shared
                    ...
                    .lock()
                    .await
                    .record(&job.npc_id, &persona.name, text);
            }
    ...
    for attempt in 0..2 {
        ...
        match reply {
            Ok(reply) => {
                ...
                match shared.validator.validate(&reply.text) {
