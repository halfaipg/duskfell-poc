...
docker run --rm -p 127.0.0.1:4107:4107 \
  -e DEPLOYMENT_PROFILE=shared-poc \
  -e PERSISTENCE_BACKEND=jsonl \
  -e ADMISSION_BACKEND=in-memory \
...
- `GET /readyz` returns readiness JSON and uses `503` when runtime dependencies, settlement queue health/capacity, durable file parent directories, durable append health, admission capacity, or drain mode are not ready. Durable path details are redacted so load-balancer-facing readiness does not expose host filesystem layout.
- `POST /api/session` issues a short-lived, single-use WebSocket session ticket. It may accept an empty body, or JSON with `Content-Type: application/json` like `{"name":"Wayfarer"}`; the server validates the display name, rejects names already pending or active on the shard, and binds the accepted name to the one-use ticket before player spawn. When `REQUIRE_ACCOUNT=true`, the request must include a valid `Authorization: Bearer ...` account token before a ticket can be minted.
- `GET /metrics` returns Prometheus-style runtime counters and gauges, including WebSocket flow, reason-specific ingress rejection counters, outbound payload sizes, interest-filtered snapshot player/object counts, admin snapshot cap state, tick/player counts, tick timing/overruns, settlement counts, settlement queue pressure/capacity, content object count, admission limits, admission rejection counters, account-auth rejections, Origin allowlist state, session issue rate limits, invalid session request bodies, display-name rejections, durable persistence failures, and pending session tickets.
- `GET /admin/summary` returns tick/player/journal/settlement/admission counters plus the loaded content manifest.
- `GET /admin/summary` includes redacted active JSONL journal and settlement outbox basenames, not absolute host paths.
...
Set `PUBLIC_DEPLOYMENT=true` for any shared or internet-reachable environment. In that mode the server refuses to boot unless `DEPLOYMENT_PROFILE=shared-poc` or `production`, `PERSISTENCE_BACKEND=jsonl`, `ADMISSION_BACKEND=in-memory`, `REQUIRE_SESSION=true`, `REQUIRE_ACCOUNT=true`, `DURABLE_SYNC_WRITES=true`, a strong bounded non-placeholder account credential for the selected `ACCOUNT_AUTH_MODE`, strong distinct bounded non-placeholder `ADMIN_TOKEN` and `METRICS_TOKEN` values, and bounded `ALLOWED_ORIGINS` are all configured, preventing the local-dev open endpoints from being exposed by accident. Public JWT mode also requires a bounded non-local HTTPS `ACCOUNT_JWT_ISSUER` without query, fragment, userinfo, whitespace, or control characters, plus a bounded printable non-placeholder `ACCOUNT_JWT_AUDIENCE`.
Set `DEPLOYMENT_PROFILE=local`, `shared-poc`, or `production` to make the intended runtime posture explicit. The default is `local`, and public deployment rejects that default so shared demos are auditable. Set `PERSISTENCE_BACKEND=jsonl` for the current PoC runtime; `postgres` is reserved for the production database/event-store path and is rejected until it is implemented. Set `ADMISSION_BACKEND=in-memory` for the current single-process admission/session/rate-limit state; `redis` is reserved for production shared admission state and is rejected until implemented. `shared-poc` requires `PUBLIC_DEPLOYMENT=true`, and `production` intentionally refuses startup until durable database/event-store, signer/indexer, and cross-process admission/rate-limit state exist. `/admin/summary` and `/metrics` expose the active profile, persistence backend, and admission backend.
...
The command verifies `PUBLIC_DEPLOYMENT=true` refuses a missing shared deployment profile, unsafe local defaults, missing persistence or admission backend, weak public tokens, placeholder public tokens, and unsynced durable writes, then starts an isolated hardened `DEPLOYMENT_PROFILE=shared-poc` / `PERSISTENCE_BACKEND=jsonl` / `ADMISSION_BACKEND=in-memory` server and checks account bearer protection before session issuance, admin token protection, metrics token protection, strict sessions, Origin allowlisting, and the public-deployment admin/metrics signal.
